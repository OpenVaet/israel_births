...
my $vaers_foreign = "raw_data/vaers_foreign";    # Where we expect to find CDC's foreign data folder.
...
parse_foreign_data();     # Parsing US data from 2020.
...
sub parse_us_data {
	for my $year (sort{$a <=> $b} keys %years) {
		...
		die "missing mandatory file for year [$year] in [$vaers_folder]" if !-f $data_file || !-f $products_file;
		%reports_products = ();

		parse_vaers_files(1, $data_file, $products_file);
	}
}
...
sub parse_foreign_data {
	...
	# Configuring expected files ; dying if they aren't found.
	my $data_file     = "$vaers_foreign/NonDomesticVAERSDATA.csv";
	my $products_file = "$vaers_foreign/NonDomesticVAERSVAX.csv";
	die "missing mandatory file for [$vaers_foreign]" if !-f $data_file || !-f $products_file;
	%reports_products = ();
...
sub parse_vaers_files {
	my ($vaers_source, $data_file, $products_file) = @_;
	...
	while (<$products_in>) {
		...
		if ($dr_num == 1) {
			...
			my $vaccine_lot  = $values{'VAX_LOT'}  // die;
